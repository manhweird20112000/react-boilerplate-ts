...
test.describe('Email Field Edge Cases', () => {
  test('Email field with spaces around valid email', async ({ page }) => {
    ...
    // The form either strips spaces or the server handles validation
    await page.waitForTimeout(500);
    
    // Verify we're still on login page (because user doesn't exist)
  ...
  test('Email field with special characters (plus sign)', async ({ page }) => {
    ...
    // Verify the email format is accepted as valid (no validation error)
    await page.waitForTimeout(500);
  ...
  test('Email field with uppercase characters', async ({ page }) => {
    ...
    // Verify the email format is accepted as valid
    await page.waitForTimeout(500);
