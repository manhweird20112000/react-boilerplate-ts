...
- [ ] UI reuses existing feature-local or `src/shared/ui` components where suitable
- [ ] New reusable primitives use the repo component stack: Base UI/Radix Slot, CVA, `cn`, Tailwind utilities, and lucide icons where appropriate
- [ ] No antd/ProComponents imports are introduced unless the dependency already exists or the user explicitly requested it
- [ ] No `.ant-*` selector or internal third-party DOM coupling is introduced
- [ ] Interactive components preserve labels, focus states, keyboard behavior, disabled/loading states, and ARIA relationships
- [ ] Styling uses Tailwind utilities/CVA variants or local component styles before global CSS
- [ ] Pages are in `pages/`
